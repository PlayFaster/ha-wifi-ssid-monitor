...
- **Service lifecycle cleanup on last entry unload (v1.5.0-dev3)**: Domain services registered in `async_setup_entry` (guarded by `has_service`) must be removed in `async_unload_entry` when the last entry is unloaded. The key subtlety: `hass.config_entries.async_entries(DOMAIN)` during `async_unload_entry` still includes the entry being unloaded (it is still in the registry at that point, just changing state). Filter it out explicitly: `remaining = [e for e in hass.config_entries.async_entries(DOMAIN) if e.entry_id != entry.entry_id]`. Only remove the service when `not remaining`.
