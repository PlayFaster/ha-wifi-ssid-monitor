...
| Scan Interval | Number | `scan_interval` | How often a scheduled scan runs. This is the only place the scan interval is set — it is no longer in the Configure dialog. |
...
| Strongest Unknown Signal | Sensor | `strongest_unknown_signal` | Signal quality of the closest unknown network, 0-100%. Higher is closer. Replaces the old dBm 'RSSI' sensor. |
