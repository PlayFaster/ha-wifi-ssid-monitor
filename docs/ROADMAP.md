...
- Every network whose band resolved is filtered out. Networks whose band did **not** resolve still pass, because `_band_allowed()` returns `True` for `None` — deliberately, so a payload change cannot silently hide everything. So the counts do not necessarily read zero; they read *wrong*, which is harder to notice.
...
- `empty_scan` does not fire either — it tests `total_aps`, which is the count *before* filtering.
